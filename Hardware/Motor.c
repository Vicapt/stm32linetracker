...
void Motor_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);						//PA4567引脚初始化为推挽输出	
...
/**
  * 函    数：直流电机设置速度
  * 参    数：直流电机序号，Speed 要设置的速度，范围：-100~100
  * 返 回 值：无
  */
void Motor_SetSpeed(int motornum , int8_t Speed)
{
	if (motornum == 1)
	{
	...
	if (motornum == 2)
	{
		...
		{
			GPIO_ResetBits(GPIOA, GPIO_Pin_6);	//PA4置低电平
			GPIO_SetBits(GPIOA, GPIO_Pin_7);	//PA5置高电平，设置方向为反转
